...
#1 배우 'GUINESS PENELOPE'가 출연한 모든 영화의 제목을 조회하시오.
-- SELECT f.title
-- FROM film f
...
#2 각 카테고리별로 몇 개의 영화가 있는지 조회하시오.
-- SELECT c.name, COUNT(fc.film_id) as number_of_films
-- FROM category c
-- JOIN film_category fc ON c.category_id = fc.category_id
-- GROUP BY c.name;

#3 고객 ID가 5인 고객의 모든 대여 기록을 조회하시오.
...
#4 가장 최근에 데이터베이스에 추가된 10개의 영화 제목을 조회하시오.
-- SELECT title
-- FROM film
...
#1 ACADEMY DINOSAUR' 영화에 출연한 모든 배우의 이름을 조회하시오.
...
#2 'ACADEMY DINOSAUR' 영화를 대여한 모든 고객의 이름을 조회하시오.
...
#3 각 고객별로 가장 최근에 대여한 영화의 제목을 조회하시오.
...
#1 가장 많이 대여된 영화의 제목과 대여 횟수를 조회하시오.
...
#1 가장 많은 수익을 낸 영화의 제목과 수익을 조회하시오.
...
#2 평균 대여 요금보다 높은 요금의 영화 제목과 요금을 조회하시오.
-- SELECT f.title, f.rental_rate
-- FROM film f
-- WHERE f.rental_rate > (SELECT AVG(rental_rate) FROM film);

#3  가장 많은 영화를 대여한 고객의 이름과 대여 횟수를 조회하시오.
...
#1 'film' 테이블에 'New Adventure Movie'라는 새로운 영화를 추가하시오.
...
#2 고객 ID가 5인 고객의 주소를 '123 New Address, New City'로 변경하시오.
-- UPDATE customer
-- SET address_id = (SELECT address_id FROM address WHERE address = '123 New Address, New City')
-- WHERE customer_id = 5;

#3 대여 ID가 200인 대여 기록의 상태를 'returned'로 변경하시오.
-- UPDATE rental
-- SET return_date = NOW()
-- WHERE rental_id = 200;

#4 배우 ID가 10인 배우의 정보를 삭제하시오.
-- DELETE FROM actor
-- WHERE actor_id = 10;